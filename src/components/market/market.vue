      ...
      <div :class="$style.backList" ref="wrapper" :style="{ height: wrapperHeight + 'px' }">
        <mt-loadmore :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" @bottom-status-change="handleBottomChange" ref="loadmore">
           <div :class="$style.cardtxt" v-for="(list,index) in lists" :key="index">
            ...
            <div style="background:#4CAF50" v-if="list.status==3 || list.status==0 " >{{list.statusd}}</div>
            <div style="background:#FF9800" v-if="list.status==1" >{{list.statusd}}</div>
            <div style="background:#CDDC39" v-if="list.status==2 " @click="appoint(list.id)">{{list.statusd}}</div>
          </div>
  ...
  methods: {
    ...
    appoint(id) {
      ...
      this.$axios({
        ...
        // eslint-disable-next-line eqeqeq
        if (res.data.status == 1) {
          let listId = res.data.result.list.seed_id
          MessageBox.confirm(res.data.result.list.message).then(action => {
