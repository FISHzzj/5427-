<template>
  <transition name="slide">
    <div :class="$style.content">
      ...
      <section :class="$style.detail" v-for="(list,index) in lists " :key="index">
        ...
        <div :class="$style.perNumer" v-if="status==1" >
          ...
          <div v-for="(item,index) in list.payment" :key="index">
            <img :src="item.thumb" alt="" @click="thumbBig(item.thumb)">
            <div>
        ...
        <InputFile title="付款凭证" @selectpic="selectImage" @thumbhandle = "imgBg" :thumb="list.thumb" :status="status" :status2="list.status2" ></InputFile>
      ...
      <div :class="$style.fangda" v-if="fangdaTrue" @click="fangdahangle"><img :src="fangdaImg" alt="" ></div>
    </div>
  </transition>
...
export default {
  ...
  methods: {
    ...
    thumbBig(src) {
      // console.log(src)
      this.fangdaImg = src
      this.fangdaTrue = true
    },
    fangdahangle() {
      this.fangdaTrue = false
    },
    imgBg(msg) {
      console.log(msg)
      this.fangdaImg = msg
